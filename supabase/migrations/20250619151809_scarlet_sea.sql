/*
  # Fix invitation and trip visibility issues

  1. Problem Analysis
    - Users are seeing invitations to themselves when they send invites to others
    - Trips are visible before accepting invitations
    - RLS policies are too permissive

  2. Changes Made
    - Fix trip RLS policies to prevent seeing trips before accepting invitations
    - Fix invitation RLS policies to prevent seeing self-invitations
    - Ensure proper filtering of invitations and trips
    - Add validation to prevent self-invitations at database level

  3. Security
    - Users can only see trips they own or have accepted invitations to
    - Users can only see invitations sent TO them, not FROM them (unless they're the trip owner)
    - Prevent self-invitations at the database level
*/
...
-- 3) Add database-level constraint to prevent self-invitations

-- Add a check constraint to prevent inviting yourself
DO $$ 
BEGIN
  -- First, check if the constraint already exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'no_self_invitation' 
    AND table_name = 'trip_invitations'
  ) THEN
    -- Add constraint to prevent self-invitations
    ALTER TABLE public.trip_invitations 
    ADD CONSTRAINT no_self_invitation 
    CHECK (
      invitee_email != (
        SELECT email FROM auth.users WHERE id = inviter_id
      )
    );
  END IF;
END $$;
