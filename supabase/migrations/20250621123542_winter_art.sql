/*
  ...
  1. Problem
    - Users are receiving invitations to themselves despite the prevent_self_invitation trigger
    - The trigger function may have timing or case sensitivity issues
    - Need to add additional validation in the invitation service

  2. Solution
    - Improve the prevent_self_invitation trigger function
    - Add better error handling and logging
    - Ensure case-insensitive email comparison
    - Add additional validation in the database function
*/
...
-- Add a database-level check constraint as a backup
-- This will catch any cases where the trigger doesn't work
DO $$ 
BEGIN
  -- Drop existing constraint if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'check_no_self_invitation' 
    AND table_name = 'trip_invitations'
  ) THEN
    ALTER TABLE public.trip_invitations DROP CONSTRAINT check_no_self_invitation;
  END IF;
  
  -- Add the constraint (this will be checked at the database level)
  ALTER TABLE public.trip_invitations 
  ADD CONSTRAINT check_no_self_invitation 
  CHECK (
    inviter_id != (
      SELECT id FROM auth.users WHERE LOWER(email) = LOWER(invitee_email)
    )
  );
END $$;
